...
| --- | --- | --- |
| `GET` | `/basic/ping` | 서버 상태 확인 (Health Check) |
| `POST` | `/session/connect` | 세션 생성 및 SID 발급 |
| `WS` | `/session/chat` | 실시간 채팅 (WebSocket) |
| `GET` | `/archive/sessions` | 전체 세션 목록 조회 |
| `GET` | `/archive/session/{sid}` | 특정 세션 상세 및 채팅 이력 조회 |
...
- **URL**: `ws://http://15.152.162.244/doq/session/chat?sid={sid}`
- **Query Params**:
  - `sid`: (필수) 세션 ID
